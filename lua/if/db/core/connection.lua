...
---@param url string
---@return string Resolved URL with environment variables expanded
function M.resolve_url(url)
  if url:match "^%$" then
    ...
    vim.notify("[if.db] Environment variable not found: " .. env_var, vim.log.levels.ERROR)
    return url
  end
  return url
end
