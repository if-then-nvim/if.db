...
local function cli_execute(url, query)
  local adapter = require "if.db.core.adapter"
...
function M.execute(url, query)
  ...
  if not ok then
    vim.notify("[if.db] Query execution failed: " .. tostring(result), vim.log.levels.ERROR)
    return ""
  end
...
function M.execute_active(query)
  ...
  if not url then
    vim.notify("[if.db] No active connection. Use :IfDb connect first.", vim.log.levels.WARN)
    return ""
  end
