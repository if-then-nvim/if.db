...
local function apply_highlights(bufnr, result, widths, has_header)
  local ns = vim.api.nvim_create_namespace "ifdb_result"
  ...
  if has_header then
    vim.api.nvim_buf_set_extmark(bufnr, ns, 0, 0, { end_row = 1, hl_group = "IfDbHeader" })
  end

  for line_num = header_offset, total_lines - 1 do
    local row_idx = line_num - header_offset + 1
    local row_hl = row_idx % 2 == 1 and "IfDbRowOdd" or "IfDbRowEven"
...
local function format_mutation_result(verb, count)
  ...
  local verb_config = {
    UPDATE = { icon = icons.mut_update, hl = "IfDbHistoryUpdate", label = "updated" },
    DELETE = { icon = icons.mut_delete, hl = "IfDbHistoryDelete", label = "deleted" },
    INSERT = { icon = icons.mut_insert, hl = "IfDbHistoryInsert", label = "inserted" },
    CREATE = { icon = icons.mut_create, hl = "IfDbHistoryCreate", label = "created" },
    DROP = { icon = icons.mut_delete, hl = "IfDbHistoryDelete", label = "dropped" },
    ALTER = { icon = icons.mut_update, hl = "IfDbHistoryAlter", label = "altered" },
...
function M.show_result(raw, elapsed)
  ...
  if is_error_result(raw) then
    ...
    vim.notify("[if.db] Query error", vim.log.levels.ERROR)
    return
  end
  ...
  if is_mutation and verb then
    ...
    require("if.db.ui.winbar").refresh_result()
