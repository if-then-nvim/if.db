...
local ns = vim.api.nvim_create_namespace "ifdb_help"
...
local function show_float(title, sections)
  ...
  for _, section_name in ipairs(section_order) do
    ...
    if entries and #entries > 0 then
      ...
      table.insert(highlights, { line = #lines - 1, col = 2, len = #section_name, hl = "IfDbHistoryHeader" })
      ...
      for _, entry in ipairs(entries) do
        ...
        table.insert(highlights, { line = #lines - 1, col = 4, len = #entry.key, hl = "IfDbKey" })
      end
    end
  end
...
function M.show_sidebar()
  ...
  add_entry(km, "toggle_types", "Toggle column types", "Actions", entries)
  ...
  table.insert(entries, { key = format_key(cfg.keymaps.close), label = "Close if.db", category = "Global" })
