...
local db_hl_map = {
  postgres = "IfDbIconPostgres",
  mysql = "IfDbIconMysql",
  mariadb = "IfDbIconMariadb",
  sqlite = "IfDbIconSqlite",
  redis = "IfDbIconRedis",
  mongodb = "IfDbIconMongodb",
}
...
local function render_columns(lines, conn_url, tbl_name, depth, schema_name)
  for _, col in ipairs(schema.get_columns(conn_url, tbl_name)) do
    local col_icon = col.is_primary and icons.column_pk or icons.column
    local type_hint = (M.show_types and col.data_type) and (" : " .. col.data_type) or ""

    lines[#lines + 1] = indent(depth) .. col_icon .. " " .. col.name .. type_hint
    M.nodes[#M.nodes + 1] = {
      type = "column",
      name = col.name,
      expanded = false,
...
---@param table_schema string|nil
---@param column_schema string
local function render_tables(lines, conn_name, conn_url, tables, depth, key_prefix, table_schema, column_schema)
  for _, tbl in ipairs(tables) do
    local tbl_expanded = M.expanded[key_prefix .. tbl.name]
    local type_icon = tbl.type == "view" and icons.view or icons.tbl

    lines[#lines + 1] = indent(depth) .. type_icon .. " " .. tbl.name
    M.nodes[#M.nodes + 1] = {
      type = tbl.type,
      name = tbl.name,
    ...
    if tbl_expanded then
      render_columns(lines, conn_url, tbl.name, depth + 1, column_schema)
    end
  end
end
...
local function render_tree()
  ...
  for _, conn in ipairs(connections) do
    ...
    local conn_icon = icons.db(db_type)
    local conn_label = " "
        ...
        if schema_db_type == "mysql" or schema_db_type == "sqlite" then
          ...
          if tables_expanded then
            render_tables(
              ...
              conn.name .. ".",
              nil,
              db_schemas[1] and db_schemas[1].name or "main"
            )
          end
        else
...
function M.refresh()
  ...
  if M.win and vim.api.nvim_win_is_valid(M.win) then
    vim.api.nvim_set_option_value("winbar", "%#IfDbHistoryHeader#" .. icons.header .. " Explorer%*", { win = M.win })
  end
...
function M.apply_highlights()
  ...
  local ns = vim.api.nvim_create_namespace "ifdb_sidebar"
  ...
  for i, line in ipairs(lines) do
    ...
    if node then
      ...
      if node.type == "connection" then
        local db_hl = db_hl_map[node.db_type]
        local default_hl = "IfDbIconDb"
        ...
        if line:find(icons.connected, 1, true) then
          ...
          if status_start then
            ...
            vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarType", line_num, status_start + #icons.connected, -1)
          end
        elseif loading_start then
          vim.api.nvim_buf_add_highlight(M.buf, ns, active_hl, line_num, icon_start, name_start)
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, name_start, loading_start - 2)
        ...
        elseif line:find(icons.idle, 1, true) then
          ...
          if status_start then
            vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarType", line_num, status_start - 1, -1)
          end
        end
      elseif node.type == "buffers" or node.type == "saved_queries" then
        local folder_hl = node.type == "buffers" and "IfDbSidebarIconNewQuery" or "IfDbSidebarIconColumn"
        vim.api.nvim_buf_add_highlight(M.buf, ns, folder_hl, line_num, icon_start, icon_start + icon_len)
        local count_pos = line:find "%(%d+%)"
        if count_pos then
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, count_pos - 2)
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarType", line_num, count_pos - 1, -1)
        else
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, -1)
        end
      elseif node.type == "new_query_action" or node.type == "open_buffer" then
        vim.api.nvim_buf_add_highlight(
          M.buf,
          ns,
          "IfDbSidebarIconNewQuery",
          line_num,
          icon_start,
          icon_start + icon_len
        )
      ...
      elseif node.type == "saved_query" then
        vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarIconColumn", line_num, icon_start, icon_start + icon_len)
        vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, -1)
      elseif node.type == "schemas" or node.type == "tables" then
        local folder_hl = node.type == "schemas" and "IfDbSidebarIconSchemas" or "IfDbSidebarIconTable"
        vim.api.nvim_buf_add_highlight(M.buf, ns, folder_hl, line_num, icon_start, icon_start + icon_len)
        local count_pos = line:find "%(%d+%)"
        if count_pos then
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, count_pos - 2)
      ...
      elseif node.type == "schema" then
        vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarIconSchema", line_num, icon_start, icon_start + icon_len)
        local count_pos = line:find "%(%d+%)"
        if count_pos then
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, count_pos - 2)
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarType", line_num, count_pos - 1, -1)
      ...
      elseif node.type == "table" or node.type == "view" then
        vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarIconTable", line_num, icon_start, icon_start + icon_len)
        vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, -1)
      elseif node.type == "column" then
        local col_hl = node.is_primary and "IfDbSidebarIconPK" or "IfDbSidebarIconColumn"
        vim.api.nvim_buf_add_highlight(M.buf, ns, col_hl, line_num, icon_start, icon_start + icon_len)
        local col_end = line:find " : "
        if col_end then
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, col_end - 1)
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarType", line_num, col_end + 2, -1)
        else
          vim.api.nvim_buf_add_highlight(M.buf, ns, "IfDbSidebarText", line_num, text_start, -1)
        end
      end
...
function M.toggle_node()
    ...
    if M.expanded[node.name] and node.name ~= connection.get_active_name() then
      ...
      if url then
        schema.get_schemas_async(url, function(schemas, err)
          if err then
            vim.notify("[if.db] Schema load error: " .. err, vim.log.levels.ERROR)
            M.is_loading = false
            M.loading_conn_name = nil
            M.refresh()
            return
          ...
          if pending == 0 then
            ...
            vim.notify("[if.db] Connected to: " .. node.name, vim.log.levels.INFO)
            return
          end
  ...
  elseif node.type == "column" then
    ...
    vim.fn.setreg('"', node.name)
    vim.notify("[if.db] Copied: " .. node.name, vim.log.levels.INFO)
    return
  end
...
function M.open_saved_query(conn_name, query_name)
  ...
  if not content then
    vim.notify("[if.db] Failed to load query: " .. (err or "unknown error"), vim.log.levels.ERROR)
    return
  end
...
function M.delete_saved_query(conn_name, query_name)
  ...
  if ok then
    vim.notify("[if.db] Deleted query: " .. query_name, vim.log.levels.INFO)
    M.refresh()
  else
    vim.notify("[if.db] Failed to delete: " .. (err or "unknown error"), vim.log.levels.ERROR)
  end
end
...
function M.setup(buf, win)
  ...
  vim.api.nvim_set_option_value("filetype", "ifdb_sidebar", { buf = buf })
  vim.api.nvim_set_option_value("buftype", "nofile", { buf = buf })
  vim.api.nvim_set_option_value("buflisted", false, { buf = buf })
  vim.api.nvim_set_option_value("modifiable", false, { buf = buf })
  vim.api.nvim_set_option_value("swapfile", false, { buf = buf })
  vim.api.nvim_buf_set_name(buf, "[if.db] Explorer")
...
function M.setup_keymaps()
  ...
  map(keymaps.toggle_types, function()
    M.show_types = not M.show_types
    M.refresh()
  end)
        ...
        vim.ui.input({
          ...
          if new_name and new_name ~= "" and new_name ~= node.name then
            ...
            if ok then
              ...
              M.refresh()
              vim.notify("[if.db] Renamed to: " .. new_name, vim.log.levels.INFO)
            else
              vim.notify("[if.db] Rename failed: " .. (err or "unknown"), vim.log.levels.ERROR)
            end
          end
        end)
      end
  ...
  map(keymaps.insert_template, function()
    ...
    if query then
      ...
      vim.notify("[if.db] Select a table or view to insert query", vim.log.levels.WARN)
    end
  end)
  ...
  map(keymaps.copy_query, function()
    ...
    if node_idx >= 1 and node_idx <= #M.nodes then
      ...
      if node.type == "saved_query" then
        ...
        vim.notify("[if.db] Select a saved query to copy", vim.log.levels.WARN)
      end
    end
  end)
  ...
  map(keymaps.paste_query, function()
    if not M.clipboard then
      vim.notify("[if.db] Nothing to paste", vim.log.levels.WARN)
      return
    end
    local conn_name = connection.get_active_name()
    ...
    vim.ui.input({
      ...
      if input and input ~= "" then
        if storage.query_exists(conn_name, input) then
          vim.notify("[if.db] Query '" .. input .. "' already exists", vim.log.levels.ERROR)
          return
        end
        local ok, err = storage.save_query(conn_name, input, M.clipboard.content)
        if ok then
          M.refresh()
          vim.notify("[if.db] Pasted: " .. input, vim.log.levels.INFO)
        else
          vim.notify("[if.db] Paste failed: " .. (err or "unknown"), vim.log.levels.ERROR)
        end
      end
    end)
  end)
  ...
  map("?", function()
    require("if.db.ui.help").show_sidebar()
  end)
end
